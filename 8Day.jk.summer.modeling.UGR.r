...
setwd("D:/OneDrive/work/research/Steelhead/Data_analysis/LST/predictions/site_regressions/all_years/summer_all_years")  
  ...
  write.table (x=pred.out,append=F,row.names=F,file="jk_pred_v_y_UGR_2013.csv",sep = ",", col.names=T)
  stats.out <- matrix(nrow=0, ncol = 10)
  colnames(stats.out) <- c("SiteName", "Year", "RMSE", "r2", "Int", "x", "x2", "z", "e", "RMSEP")
  write.table (x=stats.out, append=F, row.names=F, file="jk_pred_metrics_UGR_2013.csv", sep=",", col.names=T)

  j <- 0
  
    while(j <= 100)
      {        
        sites <- sample(SiteID.sum, 52, replace = FALSE, prob=NULL) #change sample size with each data subset
        ...
        write.table (x=pred.out,append=T,row.names=F,file="jk_pred_v_y_UGR_2013.csv",sep = ",", col.names=F)  
        ...
        write.table (x=stats.out, append=T, row.names=F, file="jk_pred_metrics_UGR_2013.csv", sep=",", col.names=F)
