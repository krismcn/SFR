...
########
  ...
  rca_elev <- read.csv("D:/OneDrive/work/research/Steelhead/spatial_data/rca_elev.csv")
   ...
   LST.in <- read.dbf("D:/OneDrive/work/research/Steelhead/spatial_data/Linear_interp/RCAzone_LST_01.dbf")
  ...
  data.sp <- read.csv("All_model_data_sp_2000.csv", header=F, stringsAsFactors = F)
  colnames(data.sp) <- c("y", "x", "z", "e", "HUC", "SiteName")
  

  data.fall <- read.csv("All_model_data_fall_2000.csv", header=F, stringsAsFactors = F)
...
#   
  ...
  varName <- paste0("Tmn_", year)
  names.out <- sprintf("%s_%03d", varName, newnamesnum)
  colnames(tLogPred.out)[3:367] <- names.out[1:365]

  write.dbf(tLogPred.out, file = "predt2000_JD_daily_Mn.dbf")
  dbf.in <- read.dbf("predt2000_JD_daily_Mn.dbf")
#####################################
