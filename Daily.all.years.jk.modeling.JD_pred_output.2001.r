...
########
  ...
  data.sp <- read.csv("All_model_data_sp_2001.csv", header=F, stringsAsFactors = F)
  colnames(data.sp) <- c("y", "x", "z", "e", "HUC", "SiteName")
  

  data.fall <- read.csv("All_model_data_fall_2001.csv", header=F, stringsAsFactors = F)
...
#   
  ...
  write.dbf(tLogPred.out, file = "predt2001_JD_daily_Mn.dbf")
