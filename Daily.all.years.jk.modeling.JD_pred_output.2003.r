...
########
  ...
  rca_elev <- read.csv("D:/OneDrive/work/research/Steelhead/spatial_data/rca_elev.csv")
   ...
   LST.in <- read.dbf("D:/OneDrive/work/research/Steelhead/spatial_data/Linear_interp/RCAzone_LST_03.dbf")
  ...
  data.sp <- read.csv("All_model_data_sp_2003.csv", header=F, stringsAsFactors = F)
  colnames(data.sp) <- c("y", "x", "z", "e", "HUC", "SiteName")
  

  data.fall <- read.csv("All_model_data_fall_2003.csv", header=F, stringsAsFactors = F)
...
#   
  ...
  write.dbf(tLogPred.out, file = "predt2003_JD_daily_Mn.dbf")
