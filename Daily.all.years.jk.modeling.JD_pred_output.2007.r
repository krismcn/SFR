...
########
  ...
  rca_elev <- read.csv("D:/OneDrive/work/research/Steelhead/spatial_data/rca_elev.csv")
   ...
   LST.in <- read.dbf("D:/OneDrive/work/research/Steelhead/spatial_data/Linear_interp/RCAzone_LST_07.dbf")
  ...
  data.sp <- read.csv("All_model_data_sp_2007.csv", header=F, stringsAsFactors = F)
  colnames(data.sp) <- c("y", "x", "z", "e", "HUC", "SiteName")
  

  data.fall <- read.csv("All_model_data_fall_2007.csv", header=F, stringsAsFactors = F)
...
###########################################
  ...
  write.table(x=coeffs_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_coeffs_Mn.csv"), sep = ",", col.names=NA)
  write.table(x=metrics_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_metrics_Mn.csv"), sep = ",", col.names=NA)
...
#   
  ...
  write.dbf(tLogPred.out, file = "predt2007_JD_daily_Mn.dbf")
