...
########
  ...
  rca_elev <- read.csv("D:/OneDrive/work/research/Steelhead/spatial_data/rca_elev.csv")
   ...
   LST.in <- read.dbf("D:/OneDrive/work/research/Steelhead/spatial_data/Linear_interp/RCAzone_LST_08.dbf")
  ...
  newnamesnum <- as.numeric(colnames(LST.in)[1:366]);
  colnames(LST.in)[1:366] <- newnamesnum;
  LST.in[,1:366] <- LST.in[,1:366]-273.15
  ...
  spring.data <- read.csv("All_model_data_sp_2008.csv", header=F, stringsAsFactors = F)
  colnames(spring.data) <- c("y", "x", "z", "e", "HUC", "SiteName")
  spring.data$SiteName <- as.character(spring.data$SiteName)

  fall.data <- read.csv("All_model_data_fall_2008.csv", header=F, stringsAsFactors = F)
...
###########################################
  ...
  coeffs_out <- data.frame(Int=numeric(2), bLST=numeric(2), bLST2=numeric(2), bJul=numeric(2), bElev=numeric(2), HUC=numeric(2))
  metrics_out <- data.frame(PRESS=numeric(2), p2=numeric(2), r2=numeric(2), RMSEP=numeric(2), RMSE=numeric(2), HUC=numeric(2))
  rownames(metrics_out) <- c("Spring", "Fall")
  rownames(coeffs_out) <- c("Spring", "Fall")
  write.table(x=coeffs_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_coeffs_Mn.csv"), sep = ",", col.names=T)
  write.table(x=metrics_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_metrics_Mn.csv"), sep = ",", col.names=T)
...
#   
  ...
  {
    maxrow <- as.numeric(which.max(x[3:368]))
    midrow <- maxrow + 1
    j <- 1
    ...
    if (x[369] == 1)
    {
      for (l in 3:maxrow)
        ...
        k <- midrow
        for (l in midrow:368)     
        {x[l] <- x[l] * coeffs.in$bLST[2] + k * coeffs.in$bJul[2] +  x[371] * coeffs.in$bElev[2] + x[l]^2 * coeffs.in$bLST2[2] + coeffs.in$Int[2]
        k <- k + 1}
    } else
      if (x[369] == 2)
      {
        for (l in 3:maxrow)
        {x[l] <- x[l] * coeffs.in$bLST[3] + j * coeffs.in$bJul[3] +  x[371] * coeffs.in$bElev[3] + x[l]^2 * coeffs.in$bLST2[3] + coeffs.in$Int[3]
         j <- j + 1}
        k <- midrow
        for (l in midrow:368)     
        {x[l] <- x[l] * coeffs.in$bLST[4] + k * coeffs.in$bJul[4] +  x[371] * coeffs.in$bElev[4] + x[l]^2 * coeffs.in$bLST2[4] + coeffs.in$Int[4]
         k <- k + 1}
      } else
        if (x[369] == 3)
        {
          for (l in 3:maxrow)
          {x[l] <- x[l] * coeffs.in$bLST[5] + j * coeffs.in$bJul[5] +  x[371] * coeffs.in$bElev[5] + x[l]^2 * coeffs.in$bLST2[5] + coeffs.in$Int[5]
           j <- j + 1}
          k <- midrow
          for (l in midrow:368)     
          {x[l] <- x[l] * coeffs.in$bLST[6] + k * coeffs.in$bJul[6] +  x[371] * coeffs.in$bElev[6] + x[l]^2 * coeffs.in$bLST2[6] + coeffs.in$Int[6]
           k <- k + 1}
        } else
          if (x[369] == 4)
          {
            for (l in 3:maxrow)
            {x[l] <- x[l] * coeffs.in$bLST[7] + j * coeffs.in$bJul[7] +  x[371] * coeffs.in$bElev[7] + x[l]^2 * coeffs.in$bLST2[7] + coeffs.in$Int[7]
             j <- j + 1}
            k <- midrow
            for (l in midrow:368)     
            {x[l] <- x[l] * coeffs.in$bLST[8] + k * coeffs.in$bJul[8] +  x[371] * coeffs.in$bElev[8] + x[l]^2 * coeffs.in$bLST2[8] + coeffs.in$Int[8]
             k <- k + 1}
            fill <- rollmean(x[3:368], 5, align = "left")
            x[180:210] <- fill[170:200]
          }
    x
  }
  ...
  write.dbf(tLogPred.out, file = "predt2008_JD_daily_Mn.dbf")
