...
########
  ...
  Log.in <- read.csv("All_sites_2009_logger.csv")
  rca_elev <- read.csv("D:/OneDrive/work/research/Steelhead/spatial_data/rca_elev.csv")
  
#   LST.in <- read.dbf("D:/OneDrive/work/research/Steelhead/spatial_data/Linear_interp/RCAzone_LST_09.dbf")
...
###########################################
  ...
  write.table(x=coeffs_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_coeffs_Mx.csv"), sep = ",", col.names=T)
  write.table(x=metrics_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_metrics_Mx.csv"), sep = ",", col.names=T)
  ...
  for (j in 1:4)
  {
    ...
    HUC.sp <- data.sp[data.sp$HUC == j,]
      ...
      write.table(x=coeffs_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_coeffs_Mx.csv"), sep = ",", col.names=F)
      
      write.table(x=metrics_out, append=T,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_metrics_Mx.csv"), sep = ",", col.names=F)
...
######################################
  ...
  write.dbf(tLogPred.out, file = "predt2009_JD_daily_Mn.dbf")
