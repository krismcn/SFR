############################################################################################################
# This set of R scripts processes NorWeST data, splits by year, and runs a by-site LOO jk on the daily summer only 
...
setwd("D:/OneDrive/work/research/CHaMP/CHaMP_data/All_CHaMP/EP_temp/")
  ...
  data_2011 <- Log.out[Log.out$Year == 2011,]
  data_2012 <- Log.out[Log.out$Year == 2012,]
  data_2013 <- Log.out[Log.out$Year == 2013,]
  
  all_data$OrigID <- as.character(all_data$OrigID)
  ID.2011 <- as.data.frame(table(data_2011$OrigID)) 
  dates <- matrix(nrow=365, ncol=2)
  dates[,1] <- 1:365
  colnames(dates) <- c("Days", "Fill")

  Log.out <- data.frame (mup = NULL)
  SiteID <- unique(data_2011$OrigID)
  SiteID <- as.matrix(SiteID)
  Log.in <- data_2011
  
  plot.window(xlim = c(1,366), ylim = c(-5,30))
  par(mfcol=c(12,15), mar=c(1,1,1,1))
  
  for (i in SiteID) 
    {
      Log.site <- Log.in[Log.in$OrigID  == i,]
      plot(Log.site$JulDay, Log.site$DailyMean, main=i)
    } 

  Days.year.table <- as.data.frame(table(Log.in$OrigID))  
  SiteID.sum <- as.matrix(ID.table[ID.table$Freq > 40, "Var1"])
  write.table (x=Days.year.table, append=F, row.names=F, file="DaysSiteYear2011.csv", sep=",", col.names=T)
