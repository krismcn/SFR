...
##########################################################
  ...
  write.csv(x=NoNA.xyz, file=paste0(basin, "_", yearPath, "_8Day_Mx_model_data_LSTn.csv"), row.names = FALSE)
...
###################################
  ...
  NoNA.xyz <- read.csv(paste0(basin, "_", yearPath, "_8Day_Mx_model_data_LSTn.csv"), stringsAsFactors = FALSE)
...
#   p2 <- MSS/TSS
  ...
  pred.out[,5] <- yearPath
  colnames(pred.out) <- c("Y", "PredY", "JulDay", "Season", "Year")
  write.table (x=pred.out,append=F,row.names=F,file=paste0("jk_pred_v_y_Mx_LSTn", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F)  
...
write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_mod_metrics_Mx_LSTn.csv"), sep = ",", col.names=NA)
