...
library(gvlma)
  ...
  LST.in.1 <- read.dbf("BNG_12_LST.dbf")
  LST.in.2 <- read.dbf("BNG_13_LST.dbf")
  LST.in <- cbind(LST.in.1[,26:47], LST.in.2[,2:25])
  ...
  write.dbf(CLST.out, file = "LST12_13_BNG_interp.dbf")

####################################################################
#This part calculates the zonal mean of the daily LST for each RCA
####################################################################

out_Preds <- read.dbf("LST12_13_BNG_interp.dbf") #use the appropriate read statement
...
write.dbf(rca_zonal, file = "LST12_13_BNG_RCA.dbf")
...
###################################
  ...
  NoNA.xyz <- read.csv(paste0(basin, "_", yearPath, "_8Day_Min_model_data_LSTn.csv"), stringsAsFactors = FALSE)
...
#   p2 <- MSS/TSS
  ...
  write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_mx_LSTn", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F) 
...
write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_mod_metrics_Min_LSTn.csv"), sep = ",", col.names=NA)
