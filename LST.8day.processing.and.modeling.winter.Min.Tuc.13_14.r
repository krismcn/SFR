...
##########################################################
  ...
  Log.in.2 <- Log.in.2[,c(4,2,3,11,12,9,10,14,15,13,6,8,16)]
                        
  colnames(Log.in.2) <- colnames(Log.in.1)
  Log.in <- rbind(Log.in.1, Log.in.2)
  colnames(Log.in)[13] <- "yrJul"
...
write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", yrPath1, "_", yrPath2, "_mod_metrics_Min.csv"), sep = ",", col.names=NA)
  ...
  plot(pred.y$PredY, pred.y$Y, pch=16, col="blue", main="Min 8-day stream temp 2013-2014", xlab="Predicted", ylab="Observed")
  abline(0,1)
...
library(classInt)
  ...
  error_pts@data <- error_pts@data[,-5] 
  error_pts@data <- error_pts@data[,-3] 
  error_pts@data <- error_pts@data[,-4] 
  error_pts@data <- error_pts@data[,-2]
  
  network <- readOGR(dsn=".", layer = netname)
  error_pts <- spTransform(error_pts, proj4string(network))
  layername <- paste0(basin, "_", yrPath1, "_", yrPath2, "_8D_winter_min_error")
  writeOGR(error_pts, dsn=".", layer=layername, driver="ESRI Shapefile")
...
setwd(shpPath)
  ...
  means <- colMeans(network@data[4:48])
  ...
  for (i in 4:48)
    {
      ...
      title("Tucannon 8-day min 2013-2014 (°C)", sub = paste0("Julian Day ", namey), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      tmp2 <- subplot(
...
system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Tuc_13_14_min.mpeg')
