...
# #########################################################
# This part reads two years of logger data (as .csv files) and formats them into a single dataset
# #########################################################
...
library(classInt)
  ...
  error_pts@data <- error_pts@data[,-3] # x2
...
setwd(shpPath)
  ...
  names.out <- colnames(network@data[3:41])
  namesnum <- as.numeric(gsub("Tmin_", "", colnames(network@data[3:41])))
  means <- colMeans(network@data[3:41])
  SDs <- colStdevs(network@data[3:41])
  yplus <- means + SDs
  yminus <- means - SDs
  ...
  for (i in 3:42)
    {
      ...
      title("Wenatchee 8-day min 2012-2013 (°C)", sub = paste0("Julian Day ", namey), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      tmp2 <- subplot(
...
system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Wen_12_13_min.mpeg')
