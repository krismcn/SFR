...
setwd(paste0(mainPath, "/", subDir, "/", longBasin))
  ...
  LST.in.1 <- read.csv(paste0("LST", yrPath1, "_", basin, "_RCA.csv"), stringsAsFactors = FALSE)
  LST.in.2 <- read.dbf(paste0("LST", yrPath2, "_", basin, "_RCA.dbf"))
  LST.in.2 <- as.numeric(LST.in.2)
...
##########################################################
  ...
  Log.in.1 <- read.csv(file = paste0(basin, "_logger_8D_", yearPath1, ".csv"), stringsAsFactors = FALSE)

  Log.in.1$yrJul <- as.numeric(sprintf("%1d%03d", yrPath1, Log.in.1$JulDay))

  Log.in.2 <- read.csv(file = paste0(basin, "_logger_8D_", yearPath2, ".csv"), stringsAsFactors = FALSE)
...
library(classInt)
  ...
  writeOGR(error_pts, dsn=".", layer=ptsname, driver="ESRI Shapefile")
...
setwd(shpPath)
  ...
  names.out <- colnames(network@data[3:46])
  namesnum <- as.numeric(gsub("Tmin_", "", colnames(network@data[3:46])))
  means <- colMeans(network@data[3:46])
  SDs <- colStdevs(network@data[3:46])
  yplus <- means + SDs
  yminus <- means - SDs
  df <- data.frame(means=means, SDs=SDs, names=namesnum)
  sequ <- c(1:44)
  namer <- sprintf('%03d', sequ)
  fix4 <- classIntervals(means, n = 10, style = "fixed",fixedBreaks=c(-1,0,1,2,3,4,6,8,10,12,14))
  ...
  for (i in 3:46)
    {
      ...
      fix3 <- classIntervals(network@data[,i], n = 11, style = "fixed",fixedBreaks=c(-1,0,1,2,3,4,6,8,10,12,14))
      fix3.colors <- findColours(fix3,pal=seis)
      ...
      legend("right", fill = attr(fix3.colors, "palette"), title="8-day Min (°C)", legend = c("-1-0","0-1","1-2","2-3","3-4","4-6","6-8","8-10","10-12","12-14"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
      legend(x=grconvertX(c(0.8, 0.95), from='npc'), 
             y=grconvertY(c(0.1, 0.25), from='npc'),, title="Model error (°C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.5), col = "gray40", cex=.5, text.col="white");
      
      
      title("Wenatchee 8-day min 2014-2015 (°C)", sub = paste0("Julian Day ", namey), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=FALSE, cex.main=0.5, cex.sub=0.5)
      tmp2 <- subplot(
...
system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png Wen_14_15_min.mpeg')
