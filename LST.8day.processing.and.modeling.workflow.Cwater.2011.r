...
#-------------------this part fills in some holes along the seam between swath 1 & 2---------
  LST.in[LST.in$ID == 608, 1:46] <- LST.in[LST.in$ID == 609, 1:46]
  LST.in[LST.in$ID == 656, 1:46] <- LST.in[LST.in$ID == 657, 1:46]
  LST.in[LST.in$ID == 704, 1:46] <- LST.in[LST.in$ID == 705, 1:46]
  LST.in[LST.in$ID == 9277, 1:46] <- LST.in[LST.in$ID == 9278, 1:46]
  LST.in[LST.in$ID == 9559, 1:46] <- LST.in[LST.in$ID == 9560, 1:46]
  LST.in[LST.in$ID == 10120, 1:46] <- LST.in[LST.in$ID == 10121, 1:46]
...
###############################
  ...
  write.dbf(CLST.out, file = paste0("LST", year, medBasin, "_interp.dbf"))
...
#################################################################
  ...
  Log.in <- read.csv("CW_CBS_2011_logger_data.csv")
