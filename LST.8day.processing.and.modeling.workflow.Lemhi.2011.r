...
#################################################################
  ...
  NoNA.xyz <- NoNA.xyz[,c(14, 2, 1, 3, 6)]
...
###################################
  ...
  rownames(metrics_out) <- c("All", "Copy")
  rownames(coeffs_out) <- c("All", "Copy")
...
setwd(paste0(mainPath, midBasin))
  ...
  elev.in <- read.csv("D:/OneDrive/work/research/CHaMP/GIS/coverages/Lemhi/Lem_rca_elev.csv")
  #elev.in <- read.csv(paste0(basin, "_rca_elev.csv"), stringsAsFactors=F)
  
  LST.in <- read.dbf(paste0("LST", yrPath, "_", modelDir, "_RCA.dbf"))

  colnames(LST.in)<-gsub("11", "", colnames(LST.in))
...
#############################
  ...
  Au.in <- read.csv(paste0(basin, "_RCA_AU_IP.csv"), stringsAsFactors = FALSE)
...
setwd(paste0(mainPath, subDir, longBasin))
  ...
  Au.in <- read.csv(paste0(basin, "_RCA_AU_STHD.csv"), stringsAsFactors = FALSE, na.strings=c("", "NA"))
...
setwd(paste0(mainPath, subDir, longBasin))
  ...
  Au.in <- read.csv(paste0(basin, "_RCA_AU_CHNK.csv"), stringsAsFactors = FALSE)
  SumSummAU <- merge(SumSumm.out, Au.in, by.x = "RCAID", by.y = "RCAID")
  colnames(SumSummAU)[1:10] <- c("RCAID", "PctDays12","PctDays13", "PctDays16", "PctDays18", "PctDays20", "PctDays22",  "MxMx", "sdMn", "MnMx")
