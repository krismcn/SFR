...
##########################################################
  ...
  yearPath <- 13
  Log.8Day.out <- read.csv(file = paste0(basin, "_", yearPath, "_Mx_8Day_logger_data.csv"))
  Logger <- na.omit(Log.8Day.out)
  Logger$yrJul <- as.numeric(sprintf("%1d%03d", Logger$year, Logger$Days))
  write.csv(x = Logger, file = paste0(basin, "_", yearPath, "_all_sites_Mx_8Day_logger_data.csv"), row.names = F)
  ...
  Log.in <- read.csv(file = paste0(basin, "_", yearPath, "_all_sites_Mx_8Day_logger_data.csv"))
...
###############################################
# if I need to remove some IDs I have no data for
###############################################
  SiteID <- SiteID[-1]
  SiteID <- as.matrix(SiteID)
  SiteID <- SiteID[-6]
...
#############################################
  ...
  write.csv(x=NoNA.xyz, file=paste0(basin, "_", yearPath, "_8Day_max_model_data.csv"), row.names = FALSE)
...
rownames(coeffs_out) <- c("Spring", "Fall")
  ...
  pred.out[,5] <- yearPath
  colnames(pred.out) <- c("Y", "PredY", "JulDay", "Season", "Year")
  write.table (x=pred.out,append=F,row.names=F,file=paste0("jk_pred_v_y_max", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F)  
  ...
  write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_max", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F) 
...
write.table(x=coeffs_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_mod_coeffs_Mx.csv"), sep = ",", col.names=NA)

write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_mod_metrics_Mx.csv"), sep = ",", col.names=NA)
...
########################################################################################################
  ...
  for (i in 1:length(rcas))  
      {
        x <- unlist(LST.elev[i,])
        maxrow <-24    #as.numeric(which.max(x[2:47]))
...
plot(newnamesnum, LogPred.out[60, 2:47])
points(newnamesnum, LogPred.out[1, 2:47], pch=16, col="blue")

LogPred.out[LogPred.out< -0.5] = -0.5
LogPred.out$Basin_RCA <- paste0("BNG_", LogPred.out$RCAID)

names.out <- sprintf("Tmx_13_%03d", newnamesnum)
colnames(LogPred.out)[2:47] <- names.out[1:46]

LogPred.out <- LogPred.out[,-48]
write.dbf(LogPred.out, file = "predt2013_BNG_8D_Mx.dbf") 
