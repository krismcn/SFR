...
popn <- "STHD"
  ...
  for (i in 3:48)
  {
    ...
    legend("right", fill = attr(fix3.colors, "palette"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.75);
    
    
    title(main = "Wenatchee 8-day mean 2014 ('C)", sub = paste0("Julian Day ", namey), line=-4, outer=TRUE)
    subplot(
  ...
  subplot(
    plot(namesnum, means, col="blue", pch=16), 
    x=grconvertX(c(0,0.25), from='npc'), 
    y=grconvertY(c(0.75, 1), from='npc'),
    size=c(1,1), vadj=0.5, hadj=0.5,inset=c(0,0), 
