...
##########################################################
  ...
  yearPath <- 13
  Log.8Day.out <- read.csv(file = paste0(basin, "_", yearPath, "_Min_8Day_logger_data.csv"), stringsAsFactors = FALSE)
  Logger <- na.omit(Log.8Day.out)
  Logger$yrJul <- as.numeric(sprintf("%1d%03d", Logger$year, Logger$Days))
  write.csv(x = Logger, file = paste0(basin, "_", yearPath, "_all_sites_Min_8Day_logger_data.csv"), row.names = F)
  ...
  Log.in <- read.csv(file = paste0(basin, "_", yearPath, "_all_sites_Min_8Day_logger_data.csv"), stringsAsFactors = FALSE)
...
rownames(coeffs_out) <- c("Spring", "Fall")
  ...
  pred.out[,5] <- yearPath
  colnames(pred.out) <- c("Y", "PredY", "JulDay", "Season", "Year")
  write.table (x=pred.out,append=F,row.names=F,file=paste0("jk_pred_v_y_min", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F)  
  ...
  write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_min", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F) 
...
write.table(x=coeffs_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_mod_coeffs_Min.csv"), sep = ",", col.names=NA)

write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_mod_metrics_Min.csv"), sep = ",", col.names=NA)
