...
###########################################################
  ...
  write.csv(x=site22, file=paste0(basin, "_", yearPath, "_8Day_Mx_model_data_site22.csv"), row.names = FALSE)
  ...
  for(m in HUCs)
    {
      ...
      write.table (x=pred.out,append=F,row.names=F,file=paste0("jk_pred_v_y_", basin, "_", yearPath, "_", m, "_Mx_sp_fall.csv"),sep = ",", col.names=F)  
...
write.csv(x=coeffs.huc, row.names=T, file = paste0("All_data_hucs_", yrPath, "_mod_coeffs_Mx.csv"))
write.csv(x=metrics.huc, row.names=T, file = paste0("All_data_hucs_", yrPath, "_mod_metrics_Mx.csv"))
...
########################################################################################################
  ...
  coeffs.in <- read.csv(paste0("All_data_hucs_", yrPath, "_mod_coeffs_Mx.csv"))
  ...
  ind <- apply(LST.elev, 1, function(x) !any(is.na(x)))
  LST.elev <- LST.elev[ind,]
  #LST.elev <- as.matrix(LST.elev)
  ...
  names.out <- sprintf("Tmx_14_%03d", newnamesnum)
  colnames(LogPred.out)[2:47] <- names.out[1:46]

  setwd(paste0(mainPath, longBasin, "/", yearPath))
  LogPred.out <- LogPred.out[,-48]
  write.dbf(LogPred.out, file = "predt2014_BNG_8D_Mx.dbf") 
...
#############################
  setwd(paste0(mainPath, longBasin, "/", yearPath, "/"))  
  site22 <- read.csv(paste0(basin, "_14_8Day_Mx_model_data_site22.csv"), stringsAsFactors=FALSE)
  data22 <- site22
  ...
  Pred.in <- read.dbf(file = "predt2014_BNG_8D_Mx.dbf")
  ...
  write.dbf(Pred.in, file = "predt2014_BNG_8D_Mx.dbf")
...
#####
Max.in <- read.dbf("predt2014_BNG_8D_Mx.dbf") 
...
colnames(MaxSumm.out) <- c("RCAID", "SuMxMx14","SuSDMx14", "SuMnMx14")

write.dbf(MaxSumm.out, file = "BNG_2014_26June_22Sept_max_summary_All.dbf")
write.csv(MaxSumm.out, file = "BNG_2014_26June_22Sept_max_summary_All.csv", row.names = F)
