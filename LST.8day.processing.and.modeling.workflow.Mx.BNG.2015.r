...
########################################################
  ...
  Log.8Day.out <- read.csv(file = paste0(basin, "_", yrPath, "_Mx_8Day_logger_data.csv"))
  Logger <- na.omit(Log.8Day.out)
  Logger$yrJul <- as.numeric(sprintf("%1d%03d", Logger$year, Logger$Days))
  write.csv(x = Logger, file = paste0(basin, "_", yrPath, "_Mx_8Day_logger_data.csv"), row.names = F)

###########################################################

  Log.in <- read.csv(file = paste0(basin, "_", yrPath, "_Mx_8Day_logger_data.csv"), stringsAsFactors = FALSE)
  ...
  write.csv(x=NoNA.xyz, file=paste0(basin, "_", yearPath, "_8Day_Mx_model_data.csv"), row.names = FALSE)
...
###################################
  ...
  write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_", basin, "_", yearPath, "_Mx_full_year.csv"),sep = ",", col.names=F)  
...
rownames(coeffs_out) <- c("Spring", "Fall")
  ...
  write.table (x=pred.out,append=F,row.names=F,file=paste0("jk_pred_v_y_", basin, "_", yearPath, "_Mx_sp_fall.csv"),sep = ",", col.names=F) 
    ...
    for(m in HUCs)
      {
        ...
        pred.out[,5] <- m
        #colnames(pred.out) <- c("Y", "PredY", "JulDay", "Season", "Year")
        write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_HUCs", basin, "_", yearPath, "_Mx_sp_fall.csv"),sep = ",", col.names=F)  
        ...
        write.table(x=coeffs_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_", m, "_mod_coeffs_Mx.csv"), sep = ",", col.names=T)
        
        write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", yearPath, "_", m, "_mod_metrics_Mx.csv"), sep = ",", col.names=T)
        
        coeffs.huc <- rbind(coeffs.huc, coeffs_out)
        metrics.huc <- rbind(metrics.huc, metrics_out)
      }

write.csv(x=coeffs.huc, row.names=T, file = paste0("All_HUCs_", yrPath, "_mod_coeffs_Mx.csv"))

write.csv(x=metrics.huc, row.names=T, file = paste0("All_HUCs_", yrPath, "_mod_metrics_Mx.csv"))
########################################################################################################
...
########################################################################################################
  ...
  coeffs.in <- read.csv(paste0("All_HUCs_", yrPath, "_mod_coeffs_Mx.csv"))
  ...
  names.out <- sprintf("Tmx_15_%03d", newnamesnum)
  colnames(LogPred.out)[2:47] <- names.out[1:46]
  
  LogPred.out <- LogPred.out[,-48]
  write.dbf(LogPred.out, file = "predt2015_BNG_8D_Mx.dbf") 
...
#_________Max_______________________ 
  ...
  Max.in <- read.dbf("predt2015_BNG_8D_Mx.dbf") 
  ...
  colnames(MaxSumm.out) <- c("RCAID", "SuMxMx15","SuSDMx15", "SuMnMx15")
  
  write.dbf(MaxSumm.out, file = paste0(basin, "_", yearPath, "_26June_22Sept_max_summary_All.dbf"))
  write.csv(MaxSumm.out, file = paste0(basin, "_", yearPath, "_26June_22Sept_max_summary_All.csv"), row.names = F)
...
write.dbf(SumSumm.out, file = paste0(basin, "_", yearPath, "_26June_22Sept_mean_summary_All.dbf"))
write.csv(SumSumm.out, file = paste0(basin, "_", yearPath, "_26June_22Sept_mean_summary_All.csv"), row.names = F)

###################################################
# Error estimates by site
###################################################
...
library(zoo)
  ...
  PredT.in <- read.dbf(paste0("predt", yearPath, "_", basin, "_8D_Mx.dbf"))
  
  SiteID <- unique(Log.in$SiteID)
  SiteID <- as.matrix(SiteID)
  
  namesnum <-as.numeric(gsub("Tmx_15_", "", colnames(PredT.in)[2:47]))
  newnamesnum <- as.numeric(sprintf("%1d%03d", yrPath, namesnum))
  
  PredT.Log.out <- data.frame (mup = NULL)
    ...
    for (i in SiteID) 
      { 
        ...
        RCAID <- ID.in$rca_id[ID.in$GRTS_== i]
        
        Pred.site <- matrix(ncol=2, nrow=46)
        Pred.site[,1] <- newnamesnum
        Pred.site[,2] <- unlist(PredT.in[PredT.in$RCAID == RCAID,2:47])
        Pred.site <- data.frame(Pred.site)
        colnames(Pred.site) <- c("yrJul", "PredT")
        
        PredT.Log.site <- merge(Pred.site, Log.site[,c(1:3,10)], by.x = "yrJul", by.y = "yrJul", all.x=TRUE, all.y = FALSE)
        
        PredT.Log.out <- rbind(PredT.Log.out, PredT.Log.site)
      }
  
  plot(PredT.Log.out$Mx8D, PredT.Log.out$PredT, pch=16, col=PredT.Log.out$SiteID, xlim=c(5,25), ylim=c(5,25))
  qplot(x = PredT, y=Mx8D, data=PredT.Log.out, color=PredT.Log.out$color)
  
  PredT.Log.out$Diff <- PredT.Log.out$PredT - PredT.Log.out$Mx8D
  plot(PredT.Log.out$yrJul, PredT.Log.out$Diff, pch=16, col=PredT.Log.out$SiteID)

  write.csv(PredT.Log.out, "pred_v_obsv_max_by_site.csv")
  legend(7,4.3,unique(PredT.Log.out$SiteID),col=1:length(PredT.Log.out$SiteID),pch=1)
  legend('topright', legend = levels(PredT.Log.out$SiteID), col = 1:3, cex = 0.8, pch = 1)
