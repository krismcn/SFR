...
#################################################################
  ...
  LST.in <- read.dbf(paste0("LST", yrPath, "_", modelDir, "_RCA.dbf"))
  ...
  ID.in <- read.csv(paste0(longBasin, "_sites_elev.csv"), stringsAsFactors=FALSE)
  
  Log.in <- read.csv(paste0(modelDir, "_", longBasin, "_logger_8Day_", yearPath, ".csv"), stringsAsFactors=FALSE)
  ...
  NoNA.xyz <- NoNA.xyz[,c(14, 2, 1, 3, 5)]
...
write.table(x=metrics_out, append=F,row.names=T, file = paste0("All_data_", basin, "_", yearPath, "_mod_metrics_Mx.csv"), sep = ",", col.names=T)
  ...
  plot(pred.y$PredY, pred.y$Y, pch=16, col="blue", main="Max 8-day stream temp 2013", xlab="Predicted", ylab="Observed")
  abline(0,1)
...
setwd(paste0(mainPath, subDir, longBasin))
  elev.in <- read.csv(paste0(longBasin, "_RCA_elev.csv"))
...
#############################
  ...
  Au.in <- read.csv(paste0(longBasin, "_RCA_AUs_IP.csv"), stringsAsFactors = FALSE)
  ...
  colnames(Max.in)<-gsub("Tmx_13_", "", colnames(Max.in))
...
######
#________All____________________  
######
...
######
  ...
  for (i in 1:length(Aus)) 
    { 
      SumAU <- SumSummAU[SumSummAU$STHD_IP == "Steelhead",]
      v12Mn = mean(unlist(SumAU$PctDays12))
      v13Mn = mean(unlist(SumAU$PctDays13))
      v16Mn = mean(unlist(SumAU$PctDays16))
      v18Mn = mean(unlist(SumAU$PctDays18))
...
#   rcas <- unique(ST_rcas$rca_id)
  ...
  AU_Code = NULL
    ...
    for (i in 1:length(Aus)) 
      { 
        SumAU <- SumSummAU[SumSummAU$CHNK_IP == "Chinook",]
        v12Mn = mean(unlist(SumAU$PctDays12))
        v13Mn = mean(unlist(SumAU$PctDays13))
        v16Mn = mean(unlist(SumAU$PctDays16))
        v18Mn = mean(unlist(SumAU$PctDays18))
      ...
      for (i in 1:length(Aus)) 
        { 
          ...
          if(ExVal >= 10)
            {
              cols <- c("red2", "gainsboro")
              pie(value, col=cols, cex.main=3.0)
              bisect.angles <- floating.pie(0,0,value, col=cols,)
              ...
              cols <- c("red2", "gainsboro")
              pie(value, clockwise = TRUE, col=cols, cex.main=3.0)
              bisect.angles <- floating.pie(0,0,value, col=c("gainsboro"), )
              pie.labels(0,0,bisect.angles,radius=1.05, c(paste0(value[1],"%")), cex=3, font=2)
            }
