...
setwd(modelPath)
  ...
  network <- readOGR(dsn=paste0(mainPath, yearPath, "_temp_CHaMP"), layer = "UGR_2014_8D_mn")
  ...
  for (i in 3:48)
    {
      ...
      legend("right", fill = attr(fix3.colors, "palette"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","18-20", "20+"), bty = "n", cex=.75);
      
      title(main = "Upper Grand Ronde 8-day mean 2014", sub = paste0("Julian Day ", namey), line=-4, outer=TRUE)
      subplot(
        plot(namesnum[1:(i-2)], means[1:(i-2)], col="blue", pch=16, bty="n", xlim=c(0,361), ylim=c(0,20)), 
        x=grconvertX(c(0,0.5), from='npc'), 
...
system("convert -delay 40 -morph 2 *.png example3.gif")
