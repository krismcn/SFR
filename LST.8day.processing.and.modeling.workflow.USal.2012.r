...
setwd("D:/OneDrive/work/research/CHaMP/CHaMP_data/EP_temp/shapes/")
  ...
  LST.in <- read.dbf("USal_12_LST.dbf")
  PointID <- LST.in[,1]
  LST.in <- LST.in[,3:48]
  ...
  write.dbf(LST.out, file = "LST12_USal_interp.dbf")
...
#################################################################
  ...
  Log.in <- read.csv("USal_2012_NorWest_logger_data.csv")
  
  
   
  LST.in <- read.dbf("LST12_USal_RCA.dbf")
...
write.csv(x=NoNA.xyz, file="USal_2012_8Day_model_data.csv", sep=",")
...
###########################
    ...
    write.table (x=pred.out,append=F,row.names=F,file="2012_pred_v_y_USal.csv",sep = ",", col.names=T)  
...
######################################
    ...
    write.table (x=pred.out,append=T,row.names=F,file="jk_pred_v_y_USal_2012_yr.csv",sep = ",", col.names=F)  
...
write.table(x=metrics_out, append=F,row.names=T, file = "All_data_2012_mod_metrics_Mx.csv", sep = ",", col.names=T)
  ...
  # went back and calc-ed the model metrics for sping & fall together:
    ...
    setwd("D:/OneDrive/work/research/CHaMP/CHaMP_data/EP_temp/USalmon/2012/")

    Log.in <- read.csv("USal_2012_8Day_model_data.csv")
    NoNA.xyz <- Log.in[,c(14, 3, 2, 4, 6)]
    colnames(NoNA.xyz) <- c("y", "x", "z", "e", "SiteName")
...
########################################################################################################
  ...
  LST.in <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/All_CHaMP/EP_temp/USalmon/LST12_USal_RCA.dbf")
  colnames(LST.in)<-gsub("X", "", colnames(LST.in))
  ...
  coeffs.in <- read.csv("All_data_2012_mod_coeffs_Mx.csv")
  ...
  write.dbf(LogPred.out, file = "predt2012_USal_8D_Max_summer.dbf") 
...
# 15Jul-31Aug ------------------------------------------------------------------
  ...
  Max.in <- read.dbf("predt2012_USal_8D_Max_summer.dbf") 
...
write.dbf(SumSumm.out, file = "USal_2012_21Jul_31Aug_max_summary_All.dbf")
write.csv(SumSumm.out, file = "USal_2012_21Jul_31Aug_max_summary_All.csv", row.names = F)
...
write.csv(SumAU.out, file = "USal_AU_2012_21Jul_31Aug_ExPcnt_summary_All.csv", row.names = F)
