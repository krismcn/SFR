...
#################################################################
  ...
  setwd(paste0(mainPath, subDir, midBasin, "/", yearPath, "/"))
...
# run the coeffs and metrics sections of both the spring and fall model sections to output for the full year
...
# spring/fall -------------------------------------------------------------
  ...
  pred.y <- predict(mod)
  pred.y[pred.y < -0.5] = -0.5
  plot(pred.y, y, main = "8-day Max Fall Leg")
  abline(0,1)
  post_mod <- summary(lm(y ~ pred.y))
...
setwd(paste0(mainPath, subDir, midBasin, "/", yearPath))
  ...
  LogPred.out <- LST.elev[,c(24:35,48,1)]
  LogPred.out[,1:12] <- 0
  rcas <- unique(LST.elev$RCAID)
  LST.sum <- LST.elev[,c(24:35, 48,1)]
...
#############################
  ...
  setwd(paste0(mainPath, subDir, midBasin))
  ...
  plot(colnames(Max.na)[1:49], Max.out[1, 1:49])
  points(colnames(Max.na)[1:49], Max.na[1, 1:49], pch=16, col="blue")
...
######
  ...
  Au.in[Au.in==" "] <- NA
  Aus <- unique(na.omit(Au.in$AU_CHNK))
  ...
  AU_Code = NULL
    ...
    for (i in 1:length(Aus)) 
      { 
        SumAU <- SumSummAU[SumSummAU$AU_CHNK == Aus[i] & SumSummAU$CHNK_IP == "Chinook",]
        v12Mn = mean(unlist(SumAU$PctDays12))
        v13Mn = mean(unlist(SumAU$PctDays13))
        v16Mn = mean(unlist(SumAU$PctDays16))
        v18Mn = mean(unlist(SumAU$PctDays18))
...
popn <- "CHNK"
    ...
    varName <- paste0("v", l, "DMax")
      ...
      for (i in 1:length(Aus)) 
        { 
          ...
          filename <- paste0(graphPath, basin, "/", yearPath, "/PctD", l, "/", name, "_", l, popn, ".png", sep="")
  ...
  for (i in 1:12)
  {
    ...
    namey <- gsub("Tmn_13_", "", colnames(network@data)[i])
    
    filename <- paste0(mainPath, "EP_temp/", basin, "/", yearPath, "/graphics/", namer[i-2], ".png", sep="")
    png(filename=filename, res = 300, width = 1500, height = 1500, units = "px", bg="black")
    
    fix3 <- classIntervals(network@data[,i], n = 8, style = "fixed",fixedBreaks=c(4,6,8,10,12,14,16,18,20))
    fix3.colors <- findColours(fix3,pal=seis)
    
    plot(network, col=fix3.colors, bg="black", fg="white")
    legend("right", fill = attr(fix3.colors, "palette"), legend = c("4-6","6-8","8-10","10-12","12-14","14-16","16-18","18+"), bty = "n", cex=.5, inset=c(.1,0), text.col="white");
    
    
    title("Methow 8-day summer max 2011-2014 ('C)", sub = paste0("Julian Day ", namey), line=-4, adj=.80, col.main="white", col.sub="white", outer=TRUE, cex.main=0.5, cex.sub=0.5)
    tmp2 <- subplot(
      plot(namesnum[1:(i)], means[1:(i)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
    ...
    par(tmp2)
    arrows(namesnum[1:(i)], yplus[1:(i)], namesnum[1:(i)], yminus[1:(i)], length=0, lwd=20, code=3, lend=0, col="gray20")
    par(op)
    tmp2 <- subplot(
      plot(namesnum[1:(i)], means[1:(i)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
      x=grconvertX(c(0.1,0.45), from='npc'), 
