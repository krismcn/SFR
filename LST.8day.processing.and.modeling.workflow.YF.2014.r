...
dataDir <- "D:/OneDrive/work/research/CHaMP/GIS/LST/LST_s1_2014/"
setwd(paste0(dataDir))


  LST.in <- read.dbf(paste0("YF_14_LST.dbf")
  GrPolID <- LST.in[,1]
  LST.in <- LST.in[,2:47]
...
###########################################
  ...
  plot(pred.y, y, main = "8-day Mean stream temperature (Spring Leg)", xlab="Predicted ('C)", ylab="Observed ('C)")
  abline(0,1)
  post_mod <- summary(lm(y ~ pred.y))
  gvmodel <- gvlma(mod)
  summary(gvmodel)
...
###########################################
  ...
  pred.out[,5] <- yearPath
  plot(pred.out[,1], pred.out[,2])
  write.table (x=pred.out,append=T,row.names=F,file=paste0("jk_pred_v_y_Mean_", basin, "_", yearPath, "_sp_fall.csv"),sep = ",", col.names=F)  
...
ptsPath <- paste0("D:/OneDrive/work/research/CHaMP/CHaMP_data/", midBasin, "/", "Mean_models")
  ...
  error_pts <- readOGR(dsn=".", "YF_Error_2014_8D_Mn")
  HUCs <- readOGR(dsn=".", "YF_HUC6")
    ...
    for (i in 3:48)
      {
        ...
        filename <- paste0(mainPath, midBasin, "/", yearPath, "/graphics3/", namer[i-3], ".png", sep="")
        ...
        cexEr <- ifelse(abs(error_pts@data[,i+1]) <= 1, 0.5,
                        ifelse(abs(error_pts@data[,i+1])>1, 0.75,
                               ifelse(abs(error_pts@data[,i+1])>2, 1.0,
                                      ifelse(abs(error_pts@data[,i+1])>3, 1.5, NA))))
        plot(HUCs, border="white", lwd=0.5, bg="black", xpd=NA)
        plot(network, col=fix3.colors, bg="black", fg="white", add=TRUE)
        points(error_pts, pch=16, col="gray40", cex=cexEr)
        
        legend("right", title="8-day Mean", fill = attr(fix3.colors, "palette"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","20-22"), bty = "n", cex=.5, inset=c(-1,0), text.col="white");
        
        legend("bottomright", title="Model error ('C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.5), col = "gray40", cex=.5, inset=c(-0.5,0), text.col="white");
        
        
        title("Yankee Fork 8-day mean 2014 ('C)", sub = paste0("Julian Day ", namey), line=-0.9, adj=.80, col.main="white", col.sub="white", outer=TRUE, cex.main=0.5, cex.sub=0.5)
        tmp2 <- subplot(
          ...
          x=grconvertX(c(0,0.40), from='npc'), 
        ...
        tmp2 <- subplot(
          plot(namesnum[1:(i-3)], means[1:(i-3)], col=fix4.colors, pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, main="Basin mean (+/-SD)", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
          x=grconvertX(c(0.0,0.40), from='npc'), 
          y=grconvertY(c(0.05, 0.20), from='npc'),
          size=c(1,1.5), vadj=0.5, hadj=0.5, 
          pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
...
setwd(paste0(mainPath, midBasin, "/", yearPath, "/graphics3/"))

system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2014_mn_w_error.mpeg')
...
library(classInt)
  ...
  netname <- paste0(basin, "_STHD_", yearPath, "_8D_Mn")
  ptsname <- paste0(basin, "_Error_", yearPath, "_8D_mn")
...
setwd(shpPath) 
  ...
  HUC <- unique(network@data$OBJECTID_2)
  names.out <- colnames(network@data[4:49])
  namesnum <- as.numeric(gsub("Tmn_14_", "", colnames(network@data[4:49])))
  HUCmeans <- data.frame(means=NULL, SDs=NULL, names=NULL)
  
  for (i in HUC)
    {
      means <- colMeans(network@data[network@data$OBJECTID_2 == i,4:49])
      SDs <- colStdevs(network@data[network@data$OBJECTID_2 == i,4:49])
      yplus <- means + SDs
      yminus <- means - SDs
      df <- data.frame(means=means, SDs=SDs, names=namesnum)
      df$HUC <- i
      assign(paste0("HUC_", i), df)
    }
  ...
  for (i in 4:49)
    {
      ...
      filename <- paste0(mainPath, midBasin, "/", yearPath, "/graphics4/", namer[i-3], ".png", sep="")
      png(filename=filename, res = 300, width = 6, height = 6, units = "in", bg="black")
      ...
      invisible(text(getSpPPolygonsLabptSlots(HUCs), labels=as.character(HUCs@data$OBJECTID_1), cex=0.6, col="white"))
      
      legend(x=grconvertX(c(.9, .95), from="npc"), 
             y=grconvertY(c(0, .5), from="npc"), title="8-day Mean ('C)", fill = attr(fix3.colors, "palette"), legend = c("0-2","2-4","4-6","6-8","8-10","10-12","12-14","14-16","16-18","20-22"), bty = "n", cex=.5, text.col="white", xpd=NA);
      
      legend(x=grconvertX(c(.91,.95), from='npc'), 
             y=grconvertY(c(0, .2), from='npc'), title="Model error ('C)", legend = c("0-1","1-2","2-3","3+"), bty = "n", pch=16, pt.cex=c(0.5, 0.75, 1.0, 1.25), col = "gray40", cex=.5, text.col="white", xpd=NA);
      
      legend(x=grconvertX(c(0.3,0.35), from='npc'), 
             y=grconvertY(c(0.0, 0.20), from='npc'), title="HUC", legend = c("1","2","3","4","5"), bty = "n", pch=c(16, 16, 18, 15, 17), col = c("blue", "yellow", "red", "lightblue", "purple"), cex=.5, text.col="white", xpd=NA);
      
      title("Yankee Fork 8-day mean 2014 ('C)", sub = paste0("Julian Day ", namey), line=-5, adj=.15, col.main="white", col.sub="white", outer=TRUE, cex.main=0.5, cex.sub=0.5)
      tmp2 <- subplot(
        plot(namesnum[1:(i-3)], HUC_1$means[1:(i-3)], col="blue", pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.0,0.3), from='npc'), 
        y=grconvertY(c(0.05, 0.20), from='npc'),
        size=c(1,2), vadj=0, hadj=0, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
      
      tmp2 <- subplot(
        plot(namesnum[1:(i-3)], HUC_2$means[1:(i-3)], col="yellow", pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.0,0.3), from='npc'), 
        y=grconvertY(c(0.05, 0.20), from='npc'),
        size=c(1,2), vadj=0, hadj=0, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
      tmp2 <- subplot(
        plot(namesnum[1:(i-3)], HUC_3$means[1:(i-3)], col="red", pch=18, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.0,0.3), from='npc'), 
        y=grconvertY(c(0.05, 0.20), from='npc'),
        size=c(1,2), vadj=0, hadj=0, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
      tmp2 <- subplot(
        plot(namesnum[1:(i-3)], HUC_4$means[1:(i-3)], col="lightblue", pch=15, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.0,0.3), from='npc'), 
        y=grconvertY(c(0.05, 0.20), from='npc'),
        size=c(1,2), vadj=0, hadj=0, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
      tmp2 <- subplot(
        plot(namesnum[1:(i-3)], HUC_5$means[1:(i-3)], col="purple", pch=17, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, main="Basin mean by HUC", adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="black"), 
        x=grconvertX(c(0.0,0.3), from='npc'), 
        y=grconvertY(c(0.05, 0.20), from='npc'),
        size=c(1,2), vadj=0, hadj=0, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
...
setwd(paste0(mainPath, midBasin, "/", yearPath, "/graphics4/"))

system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2014_mn_HUCs_error.mpeg')

###############################################
# Edited 5 may 2016 to add animation output
# Edited 25 May 2016 to add HUC display
# Edited 7 June 2016 to make the xy scatter plot the main focus
################################################
  ...
  for (i in 4:49)
    {
      ...
      filename <- paste0(mainPath, midBasin, "/", yearPath, "/graphics5/", namer[i-3], ".png", sep="")
      png(filename=filename, res = 300, width = 6, height = 6, units = "in", bg="black")
      ...
      plot(namesnum[1:(i-3)], HUC_2$means[1:(i-3)], col="yellow", pch=16, bty="n", xlim=c(0,360), ylim=c(0,18), cex.main=.8, adj=0, xlab='',ylab='', col.lab="white", cex.axis=0.5, cex.lab = 0.25, col.axis="white", col.main = "white", bg="white") 
      points(namesnum[1:(i-3)], HUC_1$means[1:(i-3)], col="blue", pch=16,) 
      points(namesnum[1:(i-3)], HUC_3$means[1:(i-3)], col="red", pch=18, bty="n",) 
      points(namesnum[1:(i-3)], HUC_4$means[1:(i-3)], col="lightblue", pch=15, bty="n",) 
      points(namesnum[1:(i-3)], HUC_5$means[1:(i-3)], col="purple", pch=17, bty="n",) 
      
      title("Yankee Fork 8-day mean 2014 ('C)", sub = paste0("Julian Day ", namey), line=-5, adj=.55, col.main="white", col.sub="white", outer=TRUE, cex.main=0.5, cex.sub=0.5)
      
      tmp2 <- subplot(
        plot(HUCs, border="white", lwd=0.5, bg="black", xpd=NA, col=c("blue", "yellow", "red", "lightblue", "purple"),),
        x=grconvertX(c(0.0,0.3), from='npc'), 
        y=grconvertY(c(0.7, 1), from='npc'),
        size=c(1,2), vadj=0, hadj=0, 
        pars=list( mar=c(0,0,0,0)+0.1, cex=0.5))
...
setwd(paste0(mainPath, midBasin, "/", yearPath, "/graphics5/"))

system('"C:/Program Files/ImageMagick-7.0.1-Q16/convert.exe" -delay 20 -morph 3 *.png YF_2014_XY_HUCs.mpeg')
