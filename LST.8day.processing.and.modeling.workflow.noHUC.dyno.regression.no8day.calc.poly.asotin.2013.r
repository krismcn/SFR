...
##############################################################################################
# This section reads in the 1km LST data for a year and fills any gaps across the year at each pixel using a linear interpolation.
##############################################################################################
...
library(pls)
	...
	LST.in <- read.csv("Aso_LST_2013.csv", header = TRUE)
	GrPolID <- LST.in[,1]
	LST.in <- LST.in[,3:48]
	...
	points(1:46, LST.out[100, 1:46], col="green")
	LST.out$GRID_CODE <- GrPolID

	newnames <- substring (colnames(LST.out)[1:46],8,10); #clips the MODIS grid names down to the julian date
...
#################################################################
setwd("D:/OneDrive/work/research/CHaMP/CHaMP_data/Asotin")
pathname <-"D:/OneDrive/work/research/CHaMP/CHaMP_data/Asotin/11Aug15_redo/"

ID.in <- read.csv("Aso_sites_GRIDCODE.csv")
#write.table(x=ID.in, append=F,row.names=F, file = "Aso_sites_elev_rca.csv", sep = ",", col.names=T)

Log.in <- read.csv("Aso_8Day_Mn_2013.csv")

 
LST.in <- read.dbf("D:/Dropbox/work/research/CHaMP/CHaMP_data/Asotin/LST13_Aso_RCA.dbf")
...
#first section has code for basin-wide model application
  ...
  setwd(pathname)
  LST.in <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/Wenatchee/LST13_Aso_RCA.dbf")
  ...
  elev.in <- read.dbf("D:/OneDrive/work/research/CHaMP/CHaMP_data/Asotin/Aso_rca_elev.dbf")
  elev.in$RCAID <- as.numeric(gsub("Asotin_", "", elev.in$BASINRCA))
  LST.elev <- merge(LST.in, elev.in, by.x = "OBJECTID", by.y = "RCAID")
  LST.elev <- LST.elev[,c(1:47,52)]
  colnames(LST.elev)[48] <- "elev"
  LST.elev$elev <- LST.elev$elev/1000
  coeffs.in <- read.csv("2013_mod_coeffs_Mn.csv")
  ...
  rcas <- unique(elev.in$RCAID)
      ...
      for (i in rcas)
        {
          x <- unlist(LST.elev[LST.elev$OBJECTID == i,2:48])
          maxrow <- 28 #as.numeric(which.max(x[1:46])) #either specify or let be dynamic
          midrow <- maxrow + 1
          day <- 225
...
#################################################################
#Summary stats - reads in a years worth of 8-day predictions and summarizes for various annual and seasonal periods
##################################################################
...
write.table(x=pred_summary, append=F,row.names=F, file = "C:/Dropbox/work/research/CHaMP/CHaMP_data/LST/%Basin%_%YY%_summary.csv", sep = ",", col.names=T)  #update naming
