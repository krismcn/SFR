...
# output ------------------------------------------------------------------
      ...
      write.table (x=pred.out,append=F,row.names=F,file="2012_pred_v_y_Tuc.csv",sep = ",", col.names=T)  
...
coeffs.in <- read.csv("All_data_2012_mod_coeffs_Mn.csv")
  ...
  for (i in 1:length(rcas))  
    {
      ...
      for (l in 1:12)
      {x[l] <- x[l] * coeffs.in$sp_bLST[5] + j * coeffs.in$sp_bJul[5] +  x[13] * coeffs.in$sp_bElev[5] + x[l]^2 * coeffs.in$sp_bLST2[5] + coeffs.in$sp_Int[5]
       j <- j + 8}
...
colnames(LogPred.out)[1:12] <- names.out[24:35]

write.dbf(LogPred.out, file = "predt2012_Tuc_8D_Mn_summer.dbf") 
